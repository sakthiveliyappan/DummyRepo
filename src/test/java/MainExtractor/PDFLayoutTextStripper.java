...
public class PDFLayoutTextStripper extends PDFTextStripper {
    ...
    */
    @Override  // This will process the contents of a single page.
    ...
    // solution : find out a way that should read bold fonts also in a line.

    
    @Override // This will print the text to the output stream.
    protected void writePage() throws IOException {
    	
        List<List<TextPosition>> charactersByArticle = super.getCharactersByArticle();
        
        for( int i = 0; i < charactersByArticle.size(); i++) {
            List<TextPosition> textList = charactersByArticle.get(i); // textList contains all the characters in page including spaces
          //  System.out.println("textList"+ textList.toString()); 
               
            for (int j = 0; j < textList.size(); j++) {
				//System.out.println("chars:"+ ":"+ j +":"+ textList.get(j));
			}     
            try {
          ...
          //  System.out.println("textList after sorting "+ textList.toString());

            for (int j = 0; j < textList.size(); j++) {
				//System.out.println("chars:"+ ":"+ j +":"+ textList.get(j)); // position changed list
			}
    ...
    private void writeToOutputStream(final List<TextLine> textLineList) throws IOException {
        for (TextLine textLine : textLineList) {
        	//System.out.println("Actual OUT: "+ textLine.getLine());
    ...
    private void iterateThroughTextList(Iterator<TextPosition> textIterator) {
        ...
        while ( textIterator.hasNext() ) {
            ...
            //  System.out.println("textPosition: "+ textPosition); // here order changed??
    ...
    private int getNumberOfNewLinesFromPreviousTextPosition(final TextPosition textPosition) {
        ...
        if ( textYPosition > previousTextYPosition && (textYPosition - previousTextYPosition > 5.5) ) {
           ...
           // if (DEBUG) 
            	//System.out.println(height + " " + numberOfLines +" :" + textPosition.toString());
